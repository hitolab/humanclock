...
rng(1);
factorList = 1:-0.05:0.05;  % From 1 to 0.05 with step -0.05
...
for choose_cnt = 2:choose_cnt_max
    ...
    for i = 1:100
        ...
        parameterSet = randsample(s, 1:70, choose_cnt);
        ...
        if amp < (25 * 0.1) || period >= 300
            ...
            if amp < (25 * 0.1) || period >= 300
                % Explore full factorList if still not oscillating
                for f = factorList
                    newparameters = DetailedModel('parametervalues');
                    ...
                    % Classification logic for Hopf or SNIC
            ...
            disp(strcat("hopf_cnt : ", num2str(hopf_cnt), "/", num2str(i), "(Num of parameters:", num2str(choose_cnt), ")"));
            disp(strcat("snic_cnt : ", num2str(snic_cnt), "/", num2str(i), "(Num of parameters:", num2str(choose_cnt), ")"));
        end
    ...
    % Plotting at each parameter size iteration
    nexttile;
    yyaxis left
...
function [amp, period] = computeAmpPeriod(val, t)
    if isempty(val)
        ...
        [pkvalue, pktime] = findpeaks(val, shortenedTime);
        if numel(pktime) <= 1
            period = 0;
        else
